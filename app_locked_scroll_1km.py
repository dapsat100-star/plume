...
st.set_page_config(page_title="Pluma Gaussiana (travado 1 km) — Scroll Sidebar", layout="wide")

# ---- Force scroll on sidebar ----
st.markdown('''
<style>
[data-testid="stSidebar"] { overflow-y: auto; max-height: 100vh; }
/* optional: nicer spacing for widgets */
[data-testid="stSidebar"] .stSlider, 
[data-testid="stSidebar"] .stNumberInput,
[data-testid="stSidebar"] .stSelectbox,
[data-testid="stSidebar"] .stCheckbox { margin-bottom: .5rem; }
</style>
''', unsafe_allow_html=True)

st.title("Pluma Gaussiana — altura da fonte, estabilidade e travamento de mapa (1 km)")
...
with st.sidebar:
    ...
    with st.expander("🖼 Renderização", expanded=False):
        extent_km = st.slider("Extensão simulada (km)", 1.0, 20.0, 5.0, 0.5)
...
if not st.session_state.source or not st.session_state.locked:
    ...
    st.markdown(f"### Fonte: **{lat:.6f}, {lon:.6f}**  — mapa travado em raio de 1 km")
    ...
    m2 = folium.Map(location=[lat, lon], zoom_start=14, tiles=None, control_scale=True)
...
st.markdown('---')
st.caption("Sidebar com rolagem + parâmetros em expansores. Mapa trava em 1 km a partir do clique.")
