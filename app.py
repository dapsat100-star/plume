...
def to_ppb(C,P,T):
    P_pa = P*100
    factor = (R_univ*T)/(M_CH4*P_pa)*1e9
    return C*factor
...
def render_ppb(A,vmin=0,vmax=450,log=False):
    ...
    N = np.clip((A-vmin)/(vmax-vmin),0,1)
    ...
    <div style="position:absolute; top:80px; right:10px; width:36px; height:340px; z-index:9999;">
      <div style="position:relative; width:36px; height:340px;">
        ...
        <div style="position:absolute; right:4px; top:-6px; font-size:12px; font-weight:600;
                    background:rgba(255,255,255,0.7); padding:1px 4px; border-radius:4px;">[ppb]</div>
...
if ss.source is None or not ss.locked:
    ...
    m0 = folium.Map(location=[-22.9,-43.2], zoom_start=15)
