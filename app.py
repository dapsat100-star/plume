...
# ================== MAPA FINAL ==================
    ...
    if ss.tle_cache and 'obs_date' in ss and 'obs_time' in ss:
        # recupera escolha do satélite do widget (qualquer chave que termine com "Satélite")
        tle_choice = None
        for key in st.sidebar.session_state.keys():
            if key.endswith("Satélite"):
                tle_choice = st.sidebar.session_state[key]; break
        ...
        if tle_choice:
            try:
                ...
                folium.Polygon(
                    ...
                    tooltip=f"GHGSat {tle_choice} • {label} • heading {heading_deg:.1f}° @ {t_utc.isoformat()}"
                ).add_to(m1)
                st.caption(f"🛰 {tle_choice} · heading {heading_deg:.1f}° (N=0°, horário) · {label}")
