...
st.set_page_config(page_title="Pluma Gaussiana (1 km lock + extent 2 km)", layout="wide")
...
st.title("Pluma Gaussiana — lock 1 km e simulação sincronizada (2 km)")

# ---------------- Session State ----------------
ss = st.session_state
ss.setdefault("source", None)          # (lat, lon)
ss.setdefault("locked", False)
ss.setdefault("last_overlay", None)    # (png_bytes, bounds, Cmax)
ss.setdefault("_trigger_update", False)
...
with st.sidebar:
    ...
    with st.expander("🌦 Meteorologia", expanded=True):
        wind_dir_deg = st.number_input("Direção do vento (graus a partir do Norte, horário)", 0, 359, 45, 1)
    ...
    with st.expander("🖼 Renderização", expanded=False):
        # UI shows extent, but we inform that when locked it's forced to 2 km
        extent_km_ui = st.slider("Extensão simulada (km) — (travado em 2 km quando o mapa estiver bloqueado)", 1.0, 20.0, 2.0, 0.5)
    ...
    if colA.button("Atualizar camada", type="primary", use_container_width=True):
        ss._trigger_update = True
    if colB.button("Selecionar novo ponto", use_container_width=True):
        ss.source = None; ss.locked = False; ss.last_overlay = None
    if colC.button("Pluma de teste", use_container_width=True):
        ss._trigger_update = True
...
def make_overlay(lat, lon, params):
    ...
    if np.count_nonzero(alpha) == 0:
        Cnorm = (C / (Cmax + 1e-15))
        idx = (Cnorm*255).astype(np.uint8); rgb = lut[idx]
        alpha = (np.sqrt(Cnorm)*255).astype(np.uint8); alpha[Cnorm<=0.001] = 0
...
if not ss.source or not ss.locked:
    ...
    if state and state.get("last_clicked"):
        ...
        ss.locked = True
        ss.last_overlay = None
        ss._trigger_update = True  # first render
        st.success(f"Fonte definida: {ss.source[0]:.6f}, {ss.source[1]:.6f}")
        st.info("Simulação sincronizada: extent = 2 km enquanto o mapa estiver travado em 1 km.")
else:
    lat, lon = ss.source
    st.markdown(f"### Fonte: **{lat:.6f}, {lon:.6f}**  — mapa travado em raio de 1 km | simulação: 2 km")
    H_eff = effective_height(H_stack, V_exit, d_stack, Tamb, Tstack, wind_speed)

    # Force extent to 2 km when locked
    extent_km = 2.0
    ...
    if ss._trigger_update or ss.last_overlay is None:
        png_bytes, sim_bounds, Cmax = make_overlay(lat, lon, params)
        ss.last_overlay = (png_bytes, sim_bounds, Cmax)
        ss._trigger_update = False

    png_bytes, sim_bounds, Cmax = ss.last_overlay

    lock_bounds = one_km_bounds(lat, lon)
    m2 = folium.Map(location=[lat, lon], zoom_start=15, tiles=None, control_scale=True)
...
st.markdown('---')
st.caption("Enquanto o mapa estiver travado em 1 km, a simulação fica em 2 km (caixa 2x2 km) centrada na fonte.")
